...
def download_image(folder_name=None, image_name=None, image_ext=None, image_url=None):
    ...
    if not os.path.exists('apparel_images/' + folder_name):
        os.makedirs('apparel_images/' + folder_name)

    try:
        with open('apparel_images/' + folder_name+'/'+image_name+image_ext, 'wb') as img_handler:
...
def download_svg_image(folder_name=None, image_name=None, image_url=None):
    ...
    try:
        ...
        renderPM.drawToFile(drawing, 'apparel_images/' + folder_name+'/'+image_name+image_ext, fmt="PNG")
...
def parse_line(json_line):
    ...
    return primary_category, sub_category, title, image_url
...
def write_data(dictionary):
    with open('./production_furniture_data.json', 'a') as f:
        f.write(json.dumps(dictionary) + "\n")


def read_data():
    with open('./furniture_data.json', 'r') as f:
        lines = f.readlines()

    for line in lines:
        try:
            json_line = json.loads(line)
            main_category, sub_category, title, img_url = parse_line(json_line)

            folder_name = main_category + '/' + sub_category
